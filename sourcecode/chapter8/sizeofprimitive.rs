...
fn main(){
  ...
  println!("type *mut u8: {}", std::mem::size_of::<*mut u8>());             // 8字节原始指针
  println!("type *mut [u8]: {}",std::mem::size_of::<*mut [u8]>());          // 16字节胖指针= 8字节指针 + 8字节长度
  println!("type *mut [u8;4]: {}",std::mem::size_of::<*mut [u8;4]>());      // 8字节原始指针
  println!("type *mut str: {}",std::mem::size_of::<*mut str>());            // 16 字节胖指针 = 8字节指针 + 8字节长度
  println!("type *mut dyn Drop: {}",std::mem::size_of::<*mut dyn Drop>());  // 16 字节胖指针 = 8字节指针 + 8字节虚拟表
  ...
  println!("value \"Hello Milly\" as a static str slice: {}", 
  std::mem::size_of_val("Hello Milly")); 
  println!("value \"Hello Milly\" as a String: {}",  
  std::mem::size_of_val("Hello Milly").to_string()); 
