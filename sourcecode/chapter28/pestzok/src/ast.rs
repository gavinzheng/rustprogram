...
pub fn generate_ast(input: &str) -> Result<File, Error> {
    let parse_tree = parse(input).map_err(Error)?;
    println!("parse_tree: {:?}", parse_tree);
